...
def my_calc_stacked_endnodes(data, distances, settings):
    ...
    distances ... distance Matrix M which contains distances between all
                  endnodes
    settings ... settings selector variable to toogle on/off the visualisations
    ...
    #raw_row, raw_column = np.where(b_dist_is_zero)
    raw_column, raw_row = np.where(b_dist_is_zero)
    raw_row = np.sort(raw_row)
    raw_column = np.sort(raw_column)
    # Combine the row(y)- and column(x)-indices in one list and sort them
    raw_combined = np.sort(np.concatenate([raw_row, raw_column]))

    # Remove duplicates: extract unique ids and calculate their occurrences
    unique, unique_occurance = np.unique(raw_combined, return_counts=True)

    print(f'   ... {len(unique)} endnodes are stacked!')

    # Create new table, first column: unique indices
    nodes_stacked = pd.DataFrame({'index': unique})

    # Convert indices to Wayelement ID
    nodes_stacked['way_ID'] = np.ceil(((nodes_stacked['index'] + 1) / 2) - 1).astype(int)

    # Convert indices to boolean indicator if it's endnode1 (true) or 2 (false)
    nodes_stacked['endnode1'] = nodes_stacked['index'] % 2 == 0

    # Return all pairs, to group them later in another function
    nodes_stacked_pairs = np.column_stack((raw_row, raw_column))
    ...
    for i_ways in range(len(data)):
        ...
        if i_stacked_nodes >= len(nodes_stacked):
            break

        # Does current way (from data_ways_selected) contain a stacked endnode?
        if i_ways + 1 == nodes_stacked['way_ID'].iloc[i_stacked_nodes]:
            ...
            if (i_stacked_nodes < numel_way_IDs - 1) and \
               (nodes_stacked['way_ID'].iloc[i_stacked_nodes] == 
                nodes_stacked['way_ID'].iloc[i_stacked_nodes + 1]):
...
##            node_id = str(line['id'])
##            UID = str(line['UID'])
##            node1_stacked = str(np.array(line['node1_stacked']).astype(int))
##            node2_stacked = str(np.array(line['node2_stacked']).astype(int))
##
##            if 'node2_stacked' in line:
##                file.write(node_id + ', ' + UID + ', ' + node1_stacked + ', ' + node2_stacked + '\n')
...
##        print(str(line['length']) + '\n')
    ...
    #export_data(data_ways_selected, "data_ways_selected_stacked_python.txt")
    #simple_export_data(data_ways_selected, "(simple)data_ways_selected_stacked_python.txt")

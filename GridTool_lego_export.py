...
def my_ask_voltage_levels(voltage_levels_sorted):
    ...
    # in the case of multiple voltage levels, split them into separate values
...
def remove_duplicates(data):
    seen_pairs = set()  # Set zur Speicherung der bereits gesehenen Paare
    cleaned_data = []   # Liste für die bereinigten Daten

    for way in data:
        # Extrahiere die relevanten Werte aus dem Eintrag
        ID_node1_final = way.get('ID_node1_final')
        ID_node2_final = way.get('ID_node2_final')

        # Erstelle ein sortiertes Tupel der beiden IDs, um Duplikate zu erkennen (unabhängig von der Reihenfolge)
        pair = tuple(sorted([ID_node1_final, ID_node2_final]))

        # Wenn das Paar noch nicht gesehen wurde, wird es zur bereinigten Liste hinzugefügt
        if pair not in seen_pairs:
            cleaned_data.append(way)
            seen_pairs.add(pair)  # Füge das Paar den gesehenen hinzu
...
def my_add_LtgsID_clone_ways(data, export_excel_country_code):
    ...
    """
    import time  # Assuming the time module is required as it was used in the original function
    ...
    # Fill blanks where the number of systems is unknown
    for i in range(num_of_ways):
        if data[i]['systems'] is None:
            data[i]['systems'] = 2

    # Initial CloneID for the original way            
    for i in range(num_of_ways):
        data[i]['LtgsID'] = LtgsID[i]
        data[i]['CloneID'] = 'c1'  
    ...
    for way in data:
        ...
        if way['systems'] == 2:
            cloned_way_b = way.copy()
            LtgsID_current = way['LtgsID']
            way['CloneID'] = f'c{current_clone_id}'
            current_clone_id += 1
            cloned_way_b['CloneID'] = f'c{current_clone_id}'
            data_new.extend([way, cloned_way_b])
            num_of_doubled_ways += 1
            ...
            cloned_way_b['CloneID'] = f'c{current_clone_id}'
            current_clone_id += 1
            cloned_way_c['CloneID'] = f'c{current_clone_id}'
            data_new.extend([way, cloned_way_b, cloned_way_c])
            num_of_tripled_ways += 2
            ...
            cloned_way_c['CloneID'] = f'c{current_clone_id}'
            current_clone_id += 1
            cloned_way_d['CloneID'] = f'c{current_clone_id}'
            data_new.extend([way, cloned_way_b, cloned_way_c, cloned_way_d])
            num_of_quadrupled_ways += 3
...
def my_export_excel_lego(data, export_excel_country_code, data_tags, way_length_multiplier):
    ...
    This function exports the data to two excel files. Every unique endnode
    will receive a NUID (unique node ID), this too will be added to the
    dataset. Columns will be created so that LEGO can
    read the excel file. 
    ...
    # Calculate R, XL, and XC based on voltage and length
    for way in data:
        length = way['length']
